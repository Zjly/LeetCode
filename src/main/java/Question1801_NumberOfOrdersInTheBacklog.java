...
public class Question1801_NumberOfOrdersInTheBacklog {
...
class Solution1801 {
	public int getNumberOfBacklogOrders(int[][] orders) {
		...
		for(int[] order : orders) {
			...
			if(order[2] == 0) {
				while(!sellPQ.isEmpty()) {
					int[] pOrder = sellPQ.peek();
					int pPrice = pOrder[0];
					int pAmount = pOrder[1];
					// can buy
					if(pPrice <= price) {
						sellPQ.poll();
						...
						if(pAmount < amount) {
							...
							if(pAmount != amount) {
								sellPQ.add(new int[]{pPrice, pAmount - amount});
							}
							amount = 0;
							break;
						}
				...
				while(!buyPQ.isEmpty()) {
					...
					if(pPrice >= price) {
						...
						if(pAmount < amount) {
							...
							if(pAmount != amount) {
								buyPQ.add(new int[]{pPrice, pAmount - amount});
							}
							amount = 0;
							break;
						}
		...
		long MOD = 1000000007;
		while(!sellPQ.isEmpty()) {
			count = (count + sellPQ.poll()[1]) % MOD;
		}

		while(!buyPQ.isEmpty()) {
			count = (count + buyPQ.poll()[1]) % MOD;
		}
