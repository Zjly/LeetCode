...
class Solution1994 {
	public int numberOfGoodSubsets(int[] nums) {
		...
		long numberOfGoodSubsets = 0;
		final long MOD = 1000000007;
		HashMap<Integer, Set<Integer>> primeNumberFactorSetHashMap = new HashMap<>();
		...
		for(int num = 2; num <= 30; num++) {
			int p = num;
			Set<Integer> primeNumberFactorSet = new HashSet<>();
			// 循环质数
			for(int primeNumber : primeNumbers) {
				if(p % primeNumber == 0) {
					p /= primeNumber;
					primeNumberFactorSet.add(primeNumber);
				}
			}

			if(p == 1) {
				primeNumberFactorSetHashMap.put(num, primeNumberFactorSet);
			}
		}

		long count = getCount(numCount, primeNumberFactorSetHashMap, new HashSet<>(), 0, MOD, 0);
		...
		return (int)((count * ratio) % MOD);
	}

	public long getCount(int[] numCount, HashMap<Integer, Set<Integer>> primeNumberFactorSetHashMap, Set<Integer> primeNumberFactorSet, long count, final long MOD, int index) {
		if(index == primeNumberFactorSetHashMap.keySet().toArray().length) {
			return count % MOD;
		}

		int number = (int)primeNumberFactorSetHashMap.keySet().toArray()[index];
		int pCount = 0;
		if(numCount[number] != 0) {
			Set<Integer> numberFactorSet = primeNumberFactorSetHashMap.get(number);
			Set<Integer> p = new HashSet<>(primeNumberFactorSet);
			p.retainAll(numberFactorSet);
			// 两个集合无交集
			if(p.isEmpty()) {
				p = new HashSet<>(primeNumberFactorSet);
				p.addAll(numberFactorSet);
				pCount += getCount(numCount, primeNumberFactorSetHashMap, p, count == 0 ? numCount[number] : count % MOD * numCount[number] % MOD, MOD, index + 1) % MOD;
			}
		}
		pCount += getCount(numCount, primeNumberFactorSetHashMap, primeNumberFactorSet, count, MOD, index + 1) % MOD;
