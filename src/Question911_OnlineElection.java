...
public class Question911_OnlineElection {
...
class TopVotedCandidate {
	...
	public TopVotedCandidate(int[] persons, int[] times) {
		...
		int maxCountCandidate = 0;
		int[] voteCount = new int[length];
		treeSet = new TreeSet<>(Comparator.comparingInt(a -> a[0]));

		for(int i = 0; i < length; i++) {
			int person = persons[i];
			voteCount[person]++;
			if(voteCount[person] >= voteCount[maxCountCandidate]) {
				maxCountCandidate = person;
			}

			treeSet.add(new int[]{times[i], maxCountCandidate});
		}
	}
